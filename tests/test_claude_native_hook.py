...
def test_evaluate_policy_user_prompt_submit_fails_closed_on_error(
    ...
    monkeypatch.setattr(
        sys,
        "stdin",
        fake_stdin(json.dumps({"hook_event_name": "UserPromptSubmit", "prompt": "hello"})),
    )
...
def test_evaluate_policy_post_tool_use_fails_open_on_error(
    ...
    # fake_stdin (not io.StringIO) so stdin has a .buffer: the hook reads
    # sys.stdin.buffer.read().decode("utf-8") (UTF-8 invariant); io.StringIO
    # has no .buffer and would AttributeError in _read_stdin_utf8.
    monkeypatch.setattr(sys, "stdin", fake_stdin(json.dumps(payload)))
